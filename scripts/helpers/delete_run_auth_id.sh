...
AUTH_RESOURCE="projects/${GOOGLE_CLOUD_PROJECT_NUMBER}/locations/global/authorizations/${RUN_AUTH_ID}"
...
if [[ "${FORCE_FLAG}" != "--force" ]]; then
  read -p "This will delete the RUN_AUTH_ID authorization. Continue? (y/N) " -r
  if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
echo ""
echo "✅ Authorization resource ${RUN_AUTH_ID} deleted."
